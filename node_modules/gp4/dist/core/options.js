...
const _USER_AGENT_TPL = () => {
    ...
    return `Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/${random(_CHROME_VERSIONS)} Safari/537.36`;
};
...
export default class Options {
    ...
    */
    ...
    In such a scenario, Gp4 will switch over to the right protocol agent for you.
    ...
    await gp4('https://sindresorhus.com', {
        agent: {
            http: new HttpAgent(),
    ...
    */
    ...
    /**
    Milliseconds to wait for the server to end the response before aborting the request with `gp4.TimeoutError` error (a.k.a. `request` property).
    ...
    */
    ...
    __Tip__: Useful when used with `Gp4.extend()` to create niche-specific Gp4 instances.
    ...
    await gp4('unicorn', {prefixUrl: 'https://cats.com'});
    //=> 'https://cats.com/unicorn'

    const instance = gp4.extend({
    ...
    */
    ...
    __Note #2__: If you provide this option, `gp4.stream()` will be read-only.
    ...
    Since Gp4 12, the `content-length` is not automatically set when `body` is a `fs.createReadStream`.
    */
    get body() {
    ...
    __Note #1__: If you provide this option, `gp4.stream()` will be read-only.
    ...
    */
    ...
    @example
    ```
    await gp4('https://example.com/?query=a b'); //=> https://example.com/?query=a%20b
    await gp4('https://example.com/', {searchParams: {query: 'a b'}}); //=> https://example.com/?query=a+b

    // The query string is overridden by `searchParams`
    await gp4('https://example.com/?query=a b', {searchParams: {query: 'a b'}}); //=> https://example.com/?query=a+b
    ```
    */
    get url() {
    ...
    */
    ...
    const request = gp4('https://httpbin.org/anything', {
    ...
    */
    ...
    await gp4('https://example.com', {searchParams});
    ...
    */
    ...
    Note that if a `303` is sent by the server in response to any request type (`POST`, `DELETE`, etc.), Gp4 will automatically request the resource pointed to in the location header via `GET`.
    ...
    */
    ...
    /**
    Determines if a `gp4.HTTPError` is thrown for unsuccessful responses.
    ...
    */
    ...
    /**
    If set to `true`, Gp4 will additionally accept HTTP2 requests.
    ...
    const {headers} = await gp4('https://nghttp2.org/httpbin/anything', {http2: true});
    ...
    */
    ...
    As the [specification](https://tools.ietf.org/html/rfc7231#section-6.4) prefers to rewrite the HTTP method only on `303` responses, this is Gp4 default behavior.
    Setting `methodRewriting` to `true` will also rewrite `301` and `302` responses, as allowed by the spec. This is the behavior followed by `curl` and browsers.

    __Note__: Gp4 never performs method rewriting on `307` and `308` responses, as this is [explicitly prohibited by the specification](https://www.rfc-editor.org/rfc/rfc7231#section-6.4.7).
    ...
    */
    ...
    const parsed = await gp4('https://example.com', {
    ...
    */
    ...
    await gp4.post('https://example.com', {
    ...
    */
    ...
    set https(value) {
        ...
        // (Fixed)
        this._internals.https = this._merging  // Fixed typo "_marging" -> "_merging"
    ...
    __Note__: This doesn't affect streams! Instead, you need to do `gp4.stream(...).setEncoding(encoding)`.
    ...
    */
    ...
    This is equivalent to calling `gp4.stream(url, options?)`.
    ...
    */
    ...
    /**
    function to `decode` html `entities` code in a simple way
    ...
    (async () => {
        ...
        const { body } = await gp4.get(url, {
            decode: true,
        })

        const regex = /<a\s+href="https:\/\/x\.com\/github"\s+class="footer-social-icon\s+d-block\s+Link--outlineOffset"\s+data-analytics-event="([\s\S]+?)"\s*>/;

        console.log(body.match(regex)[1]);
    })
    ```
    example of decode results

    decode: true

    {"category":"Footer","action":"go to X","label":"text:text:x"}

    Example if using default without decode
    ...
    */
    get decode() {
        return this._internals.decode;
    }
    set decode(value) {
        this._internals.decode = value === true;
    }
    get encode() {
        return this._internals.encode;
    }
    set encode(value) {
        this._internals.encode = value === true;
    }
    /**
    The parsing method.

    The promise also has `.text()`, `.json()` and `.buffer()` methods which return another Gp4 promise for the parsed body.

    It's like setting the options to `{responseType: 'json', resolveBodyOnly: true}` but without affecting the main Gp4 promise.

    __Note__: When using streams, this option is ignored.

    @example
    ```
    const responsePromise = gp4(url);
    const bufferPromise = responsePromise.buffer();
    const jsonPromise = responsePromise.json();

    const [response, buffer, json] = Promise.all([responsePromise, bufferPromise, jsonPromise]);
    // `response` is an instance of Gp4 Response
    ...
    ```
    // This
    const body = await gp4(url).json();

    // is semantically the same as this
    const body = await gp4(url, {responseType: 'json', resolveBodyOnly: true});
    ```
    */
    get responseType() {
    ...
    createNativeRequestOptions() {
        ...
        const agent = url.protocol === 'https:'
            ? internals.http2 ? internals.agent : internals.agent.https
